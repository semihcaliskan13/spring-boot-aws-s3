...
public class FileServiceImpl implements FileService {
    ...
    @Override
    public List<FileDto> getBucketfiles(String bucketName) {
        if (!amazonS3.doesBucketExistV2(bucketName)) {
            log.error("No Bucket Found");
            return null;
        }
        return amazonS3.listObjectsV2(bucketName).getObjectSummaries().stream()
                .map(file -> new FileDto(file.getKey(), file.getSize(), file.getETag()))
    ...
    @Override
    public FileDto downloadFile(String bucketName, String fileName) {
        if (!amazonS3.doesBucketExistV2(bucketName)) {
            log.error("No Bucket Found");
            return null;
        }
